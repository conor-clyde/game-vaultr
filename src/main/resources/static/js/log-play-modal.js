(function () {
    ...
    function closeLogPlayModal() {
        ...
        document.querySelectorAll('#logPlayModal .js-log-play-submit').forEach(function (btn) {
            btn.disabled = false;
    ...
    function readPlaythroughCountHintFromButton(btn) {
        ...
        var raw = String(btn.getAttribute('data-log-playthrough-count') || '').trim();
        if (raw === '') {
            return null;
        }
    ...
    function coalescePlaythroughCountHintFromButton(btn, fromBtn) {
        ...
        var pref = String(btn.getAttribute('data-log-preferred-playthrough-id') || '').trim();
    ...
    function applyPlaythroughOptionSelection(optBtn) {
        ...
        if (triggerPt) {
            triggerPt.setAttribute('aria-label', 'Playthrough: ' + labelText + '. Open to change.');
        }
        try {
    ...
    function wireLogPlayPlaythroughDropdown() {
        ...
        panel.addEventListener('click', function (e) {
            var opt = e.target && e.target.closest ? e.target.closest('.dropdown-option') : null;
    ...
    function showPlaythroughPickerWithPreferred(items, preferredPlaythroughId, fetchOk, playthroughCountHint) {
        var wrap = $('logPlayPlaythroughPicker');
        var hidden = $('logPlayPlaythroughValue');
        var trigger = $('logPlayPlaythroughTrigger');
        var labelEl = $('logPlayPlaythroughLabel');
        ...
        if (hint === undefined || hint === null || (typeof hint === 'number' && isNaN(hint))) {
            hint = null;
        } else {
    ...
    function loadAndApplyPlaythroughs(apiId, preferredPlaythroughId, playthroughCountHint) {
        var aid = apiId != null ? String(apiId).trim() : '';
        if (!aid) {
            clearPlaythroughPicker();
            return;
        }
        fetchPlaythroughsForGame(aid, false).then(function (result) {
            ...
            var items = result.items || [];
            var fetchOk = result.ok === true;
            showPlaythroughPickerWithPreferred(items, preferredPlaythroughId, fetchOk, playthroughCountHint);
        });
    }
    ...
    function syncEndSessionRecordedHint() {
        ...
        var ref =
            typeof state.endSessionReferenceMinutes === 'number' && !isNaN(state.endSessionReferenceMinutes)
    ...
    function applyLogPlayDetailsUi() {
        ...
        if (isEdit) {
            ...
            panel.hidden = false;
            document.querySelectorAll('#logPlayModal .log-play-end-optional-details').forEach(function (el) {
                el.hidden = false;
            });
        } else {
            if (modal) {
                ...
                modal.classList.toggle('log-play-modal--end-start-unlocked', isEnd && state.endSessionStartUnlocked);
            }
            panel.hidden = false;
            document.querySelectorAll('#logPlayModal .log-play-end-optional-details').forEach(function (el) {
                el.hidden = false;
    ...
    function syncLogPlayExpSelection(activeValue) {
        ...
        var v = activeValue === undefined || activeValue === null ? 'OKAY' : String(activeValue);
        if (hidden) {
            hidden.value = v;
        }
        document.querySelectorAll('#logPlayModal .js-log-play-exp').forEach(function (btn) {
    ...
    function clearSessionRangeFields() {
        ['logPlaySessionStartDate', 'logPlaySessionEndDate', 'logPlaySessionStartTime', 'logPlaySessionEndTime'].forEach(
            function (rid) {
                var el = $(rid);
                if (el) {
                    el.value = '';
    ...
    function parseDurationMinutesFromRangeFields() {
        ...
        if (isNaN(t0) || isNaN(t1)) {
            return { ok: false, msg: 'Could not read session start and end. Check the times.' };
        }
        if (t1 <= t0) {
            if (sd === ed && t1 < t0) {
                t1 += 86400000;
    ...
    function resetForm() {
        ...
        if (durH) durH.value = '';
        if (durM) durM.value = '';
        ['logPlaySessionStartDate', 'logPlaySessionStartTime', 'logPlaySessionEndDate', 'logPlaySessionEndTime'].forEach(
            function (rid) {
                var re = $(rid);
                if (re) {
                    re.value = '';
    ...
    function applyLogRowToForm(row) {
        ...
        var startIso = row.sessionStartedAtIso != null ? String(row.sessionStartedAtIso).trim() : '';
        ...
        if (startIso) {
            ...
            if (dm != null && dm > 0 && !awaiting) {
                var playedIso = row.playedAtIso != null ? String(row.playedAtIso).trim() : '';
            ...
            if (!shouldUseRangeMode) {
                ...
                if (dm != null && dm > 0) {
                    ...
                    if (durM) durM.value = mStart > 0 ? String(mStart) : hStart > 0 ? '0' : '';
                }
            }
        ...
        if (row.id != null) {
            ...
            state.editSavedDurationMinutes =
                dmRow != null && dmRow > 0 ? dmRow : 0;
            var pti = row.playthroughId;
            state.editSavedPlaythroughId = pti != null && pti !== undefined ? pti : null;
        } else {
            state.editSavedDurationMinutes = null;
            state.editSavedPlaythroughId = null;
        }
    ...
    function getBaselinePlaythroughMinutesForPreview() {
        ...
        if (
            ...
            var sub =
                typeof state.editSavedDurationMinutes === 'number' && !isNaN(state.editSavedDurationMinutes)
                    ? state.editSavedDurationMinutes
                    : 0;
    ...
    function updateLogPlayTotalPreview() {
        ...
        side.hidden = false;
        var base =
            typeof state.baselineDisplayMinutes === 'number' && !isNaN(state.baselineDisplayMinutes)
    ...
    function openLogPlayModalForEndSession(opts) {
        ...
        var title = opts && opts.title ? String(opts.title) : '';
        var row = opts && opts.row ? opts.row : null;
        var startEpochMs = opts && opts.startEpochMs != null ? Number(opts.startEpochMs) : null;
        var m = $('logPlayModal');
        if (!m || !apiId || !row || row.id == null) {
            return;
        }
        ...
        if (state.endSessionStartEpochMs == null) {
            var parsed = parseIsoToDateAndTime(row.sessionStartedAtIso != null ? String(row.sessionStartedAtIso) : '');
        ...
        if (state.endSessionStartEpochMs != null && isFinite(state.endSessionStartEpochMs)) {
            var elapsedMin = Math.floor((Date.now() - state.endSessionStartEpochMs) / 60000);
    ...
    function validateRangeModeForSubmit() {
        ...
        if (partialStart || partialEnd) {
            return { ok: false, msg: 'Finish entering start and end time, or leave end blank for start only.' };
        }
        var any = sd !== '' || st !== '' || ed !== '' || et !== '';
        if (!any) {
            return { ok: false, msg: 'Enter session start and end, or start only to save and add the end later.' };
        }
        if (hasStart && hasEnd) {
            return parseDurationMinutesFromRangeFields();
        }
        if (hasStart && !hasEnd) {
            var t0 = localDateTimeToMs(sd, st);
            if (isNaN(t0)) {
                return { ok: false, msg: 'Could not save that session start. Check the start time.' };
            }
            return { ok: true };
        }
        if (hasEnd && !hasStart) {
            ...
            var t1 = localDateTimeToMs(ed, et);
            if (isNaN(t1)) {
                return { ok: false, msg: 'Could not save that session time. Check the end time (after start).' };
            }
            return { ok: true };
        }
        return { ok: false, msg: 'Enter session start and end, or start only to save and add the end later.' };
    ...
    function renderGameList(games, filter) {
        ...
        filtered.forEach(function (g) {
            ...
            btn.setAttribute('data-log-baseline-minutes', String(Math.max(0, Math.floor(dpm))));
            ...
            var calPick =
                g.lastPlayedCalendarLine != null && String(g.lastPlayedCalendarLine).trim()
    ...
    function focusFirstFormControl() {
        setTimeout(function () {
            try {
                ...
                var target =
                    isEdit && edit && !edit.disabled ? edit : primary && !primary.disabled ? primary : edit || primary;
                if (target) {
                    target.focus();
                }
            } catch (e) { /* no-op */ }
    ...
    function enterFormFromPick(apiId, title, pickBtn) {
        ...
        updateLogPlayTotalPreview();
        applyLogPlayDetailsUi();
        loadAndApplyPlaythroughs(apiId, null, readPlaythroughCountHintFromButton(pickBtn));
        focusFirstFormControl();
    }
            ...
            .then(function (row) {
                ...
                var startEpochRaw =
                    openerBtn && openerBtn.getAttribute
                        ? String(openerBtn.getAttribute('data-open-session-start-epoch-ms') || '').trim()
                        : '';
                var startEpoch =
                    startEpochRaw !== '' && !isNaN(Number(startEpochRaw)) ? Number(startEpochRaw) : null;
                if (startEpoch != null && isFinite(startEpoch) && Date.now() - startEpoch < 60000) {
                    return;
                }
                var baseRaw = String(openerBtn.getAttribute('data-log-baseline-minutes') || '').trim();
    ...
    function openFromButton(btn) {
        ...
        var openSessionIdRaw = String(btn.getAttribute('data-log-open-session-id') || '').trim();
        var openSessionId = openSessionIdRaw !== '' && !isNaN(Number(openSessionIdRaw)) ? Number(openSessionIdRaw) : null;
        ...
        if (openSessionId != null) {
            var startGateRaw = String(btn.getAttribute('data-open-session-start-epoch-ms') || '').trim();
            var startGateMs =
                startGateRaw !== '' && !isNaN(Number(startGateRaw)) ? Number(startGateRaw) : null;
            if (startGateMs != null && isFinite(startGateMs) && Date.now() - startGateMs < 60000) {
                return;
            }
        ...
        applyLogPlayDetailsUi();
        var baseRaw = String(btn.getAttribute('data-log-baseline-minutes') || '').trim();
        state.baselineDisplayMinutes = baseRaw === '' ? 0 : parseInt(baseRaw, 10) || 0;
        updateLogPlayTotalPreview();
        openLogPlayModal();
        var ptPrefRaw = String(btn.getAttribute('data-log-preferred-playthrough-id') || '').trim();
        var ptPref = ptPrefRaw !== '' && !isNaN(Number(ptPrefRaw)) ? ptPrefRaw : null;
        loadAndApplyPlaythroughs(
            apiId,
            ptPref,
            coalescePlaythroughCountHintFromButton(btn, readPlaythroughCountHintFromButton(btn))
        );
        focusFirstFormControl();
    }
    ...
    function initLogPlayModal() {
        ...
        document.querySelectorAll('#logPlayModal .js-log-play-duration-quick').forEach(function (btn) {
    ...
    function updateHeroLogPlayButtonState() {
        ...
        var raw = String(btn.getAttribute('data-open-session-start-epoch-ms') || '').trim();
        var openId = String(btn.getAttribute('data-log-open-session-id') || '').trim();
        ...
        if (gateEndPlay) {
            ...
            btn.disabled = !canEnd;
            btn.title = canEnd ? 'End active session' : 'Available after 1:00 on the timer';
        }
    }
    ...
    window.cgSubmitStartPlayingNowFromHero = function (apiId) {
        ...
        form.method = 'POST';
        form.action = base + '/' + encodeURIComponent(String(apiId).trim()) + '/play-log';
    ...
    window.playstateOpenEndSessionFromFloat = function () {
        ...
        var title = String(root.getAttribute('data-session-game-title') || '').trim();
        var logId = logIdRaw !== '' && !isNaN(Number(logIdRaw)) ? Number(logIdRaw) : null;
