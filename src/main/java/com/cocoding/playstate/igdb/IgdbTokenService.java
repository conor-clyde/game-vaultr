  ...
  public synchronized String getAccessToken() {
    ...
    if (normalizedClientId.isBlank() || normalizedClientSecret.isBlank()) {
      throw new RuntimeException(
          "IGDB credentials are missing. Set IGDB_CLIENT_ID and IGDB_CLIENT_SECRET (or TWITCH_*"
              + " variants).");
    }

    String requestBody =
        "client_id="
            + urlEncode(normalizedClientId)
            + "&client_secret="
            + urlEncode(normalizedClientSecret)
            + "&grant_type="
    ...
    try {
      ...
      logger.error(
          "Failed to obtain IGDB access token - HTTP error: {} - Response: {}",
          e.getStatusCode(),
