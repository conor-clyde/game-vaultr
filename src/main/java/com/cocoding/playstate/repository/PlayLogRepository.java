...
public interface PlayLogRepository extends JpaRepository<PlayLog, Long> {
  ...
  Page<PlayLog> findByUserIdAndGameIdOrderByPlayedAtDescIdDesc(
      String userId, Long gameId, Pageable pageable);
  ...
  @Modifying
  @Query(
      "UPDATE PlayLog p SET p.playthroughId = NULL WHERE p.userId = :userId AND p.gameId = :gameId")
  void clearPlaythroughIdForUserAndGame(
      @Param("userId") String userId, @Param("gameId") Long gameId);
}
