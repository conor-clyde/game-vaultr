...
import com.cocoding.playstate.dto.search.SearchResult;
import com.cocoding.playstate.dto.search.SearchSessionState;
...
public class SearchService {
  ...
  private static final double POPULAR_BADGE_MIN_SCORE =
      0.5 * 0.000248440082941 + 0.5 * 0.000297412905334;
  ...
  private static final SearchResult EMPTY_SEARCH =
      new SearchResult(Collections.emptyList(), false, null);
  ...
  private final Cache<String, List<SearchGameRow>> mergedWindowCache =
      Caffeine.newBuilder()
  ...
  private List<SearchGameRow> buildMergedResults(
      String query, SearchSessionState filters, String sort) {
  ...
  private List<SearchGameRow> computeSearchResultList(
      String query, SearchSessionState filters, String sort) {
    List<Map<String, Object>> raw =
        igdbService.searchGames(
            query, MERGED_FETCH_LIMIT, 0, filters.platforms, filters.yearMin, filters.yearMax);
    List<SearchGameRow> games =
        SearchGameRow.fromIgdbMaps(raw != null ? raw : Collections.emptyList());
    if (games.isEmpty()) {
      return games;
    }
  ...
  private static String mergedWindowCacheKey(
      String query, SearchSessionState filters, String sort) {
    String platformsKey =
        filters.platforms.stream()
  ...
  private void attachDiscoverySignals(List<SearchGameRow> games) {
    ...
    Instant newReleaseCutoff =
        Instant.now().minus(DISCOVERY_NEW_RELEASE_LOOKBACK_DAYS, ChronoUnit.DAYS);
    for (SearchGameRow g : games) {
  ...
  private void sortByRelevance(List<SearchGameRow> games) {
    ...
    games.sort(
        Comparator.comparingInt(SearchService::relevancePrimaryGroup)
  ...
  private static void sortByPopularity(List<SearchGameRow> games) {
    games.sort(
        Comparator.comparingDouble((SearchGameRow g) -> -(g.popScore != null ? g.popScore : 0d))
            .thenComparingLong(g -> g.id));
  }
  ...
  private static void sortByReleaseNewest(List<SearchGameRow> games) {
    games.sort(
        Comparator.comparingLong(
                (SearchGameRow g) -> g.firstReleaseDateEpoch != null ? g.firstReleaseDateEpoch : 0L)
            .reversed());
  }
  ...
  private void markGamesInCollection(List<SearchGameRow> games, String libraryUserId) {
    ...
    Set<String> inCollectionApiIds =
        userGameRepository.findByUserIdAndGame_ApiIdIn(libraryUserId.trim(), apiIds).stream()
