...
public class UserGame {
    ...
    public List<String> getReflectionTagsDisplay() {
        return getReflectionTags().stream()
                .map(
                        t -> ReflectionTagsJson.toDisplayLabel(ReflectionTagsJson.normalizeTag(t, 30)))
                .filter(s -> !s.isEmpty())
                .toList();
    }
    ...
    public List<WhyPlaying> getWhyPlayings() {
        ...
        for (String part : whyPlayingCsv.split(",")) {
            ...
            if (resolved != null
                    ...
                    && out.size() < WhyPlaying.MAX_PER_GAME) {
                out.add(resolved);
            }
        }
    ...
    public static int computeDisplayPlayMinutes(UserGame ug, int logSumMinutes) {
        ...
        int anchor = ug.getPlayTimeManualAnchorLogMinutes() != null ? ug.getPlayTimeManualAnchorLogMinutes() : 0;
