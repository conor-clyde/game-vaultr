...
import com.cocoding.playstate.dto.HomeDashboardDtos.DashboardHeroCopy;
import com.cocoding.playstate.dto.HomeDashboardDtos.HomeHeroQuickRow;
import com.cocoding.playstate.dto.HomeDashboardDtos.HomeHeroStats;
import com.cocoding.playstate.dto.HomeDashboardDtos.HomeInProgressTile;
import com.cocoding.playstate.dto.HomeDashboardDtos.HomePreviewTile;
import com.cocoding.playstate.dto.HomeDashboardDtos.HomeSessionRow;
...
public class HomeController {
    ...
    private void populateDashboard(Model model, Authentication authentication) {
        ...
        for (PlayLog pl : recentLogs) {
            ...
            com.cocoding.playstate.model.Game g = ug != null ? ug.getGame() : gameRepository.findById(pl.getGameId()).orElse(null);
            if (g == null) {
                continue;
            }
        ...
        int recentPlayOverflowCount = (int) Math.max(0L, totalPlayLogs - ACTIVITY_RECENT_PLAY_DISPLAY_LIMIT);
        ...
        int playingNow = (int) library.stream().filter(ug -> ug.getStatus() == GameStatus.PLAYING).count();
        int finished = (int) library.stream().filter(ug -> ug.getStatus() == GameStatus.FINISHED).count();
        ...
        model.addAttribute(
                "heroQuickRows",
                buildRecentlyPlayedHeroRows(recentLogs, byGameId, gameRepository));
        model.addAttribute(
                "dashHeroCopy",
    ...
    private DashboardHeroCopy buildDashboardHeroCopy(
        ...
        com.cocoding.playstate.model.Game g = ug != null ? ug.getGame() : gameRepository.findById(latest.getGameId()).orElse(null);
        if (g == null) {
            return new DashboardHeroCopy(DashboardHeroCopy.Phase.ACTIVE_LAST_PLAYED, null, null);
        }
    ...
    private static String computeAvgRatingDisplay(List<UserGame> library) {
        List<Integer> ratings = library.stream()
    ...
    private static List<HomeHeroQuickRow> buildRecentlyPlayedHeroRows(
        ...
        for (Long gid : pickOrder) {
            ...
            UserGame ug = byGameId.get(gid);
            com.cocoding.playstate.model.Game g = ug != null ? ug.getGame() : gameRepository.findById(gid).orElse(null);
